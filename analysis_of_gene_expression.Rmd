...
```{r, message=FALSE}
res <- results(dds.collapse, alpha = 0.05)
...
```{r message=FALSE}
library(edgeR)
condition <- rep(c(rep(2, times=3), rep(1, times=2)), times = 3)
...
deseq.volcano <- function(res, datasetName) {
  return(EnhancedVolcano(res, x = 'log2FoldChange', y = 'padj',
                         lab=rownames(res),
                         title = paste(datasetName, "Mild vs Severe"),
...
```{r}
res_top <- na.omit(as.data.frame(res))
res_top <- res_top[res_top$padj < 0.05 & res_top$log2FoldChange < -2 |res_top$log2FoldChange  > 2,]
distances <- di
pheatmap(res_top$log2FoldChange)
```
## PCA
```{r}
rlog_tf <- rlog(dds)
plotPCA(rlog_tf, intgroup="Condition")
```
